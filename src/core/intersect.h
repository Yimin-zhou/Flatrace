...
namespace core
{
    ...
    inline void intersect4x4(const Triangle &triangle, Ray4x4 &rays)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            //  }
            const Vec3_x8 tv = {
                    _mm256_sub_ps(_mm256_load_ps(rays.ox_x8.data() + i * 8),
                                  _mm256_broadcast_ss(&triangle.vertices[0].x)),
                    _mm256_sub_ps(_mm256_load_ps(rays.oy_x8.data() + i * 8),
                                  _mm256_broadcast_ss(&triangle.vertices[0].y)),
                    _mm256_sub_ps(_mm256_load_ps(rays.oz_x8.data() + i * 8),
                                  _mm256_broadcast_ss(&triangle.vertices[0].z))
            };
            ...
            const __m256i ray_triangle = _mm256_load_si256(
                    reinterpret_cast<const __m256i *>(rays.triangle.data() + rays.n * 16 + i * 8));
...
//      const __m256i new_triangle =  _mm256_set1_epi32(triangle.id);
                const __m256i new_triangle = _mm256_castps_si256(_mm256_blendv_ps(_mm256_castsi256_ps(ray_triangle),
                                                                                  _mm256_castsi256_ps(_mm256_set1_epi32(
                                                                                          triangle.id)), update_rays));

                _mm256_store_ps(rays.t.data() + rays.n * 16 + i * 8, new_t);
                _mm256_store_ps(rays.dot.data() + rays.n * 16 + i * 8, new_dot);
                _mm256_store_si256(reinterpret_cast<__m256i *>(rays.triangle.data() + rays.n * 16 + i * 8),
                                   new_triangle);
            }
        }
    }
    ...
    inline float intersect4x4(const BoundingBox &bbox, const Ray4x4 &rays)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            const __m256 tx1 = _mm256_mul_ps(
                    _mm256_sub_ps(_mm256_broadcast_ss(&bbox.min.x), _mm256_load_ps(rays.ox_x8.data() + i * 8)),
                    ray_rd_x);
            const __m256 tx2 = _mm256_mul_ps(
                    _mm256_sub_ps(_mm256_broadcast_ss(&bbox.max.x), _mm256_load_ps(rays.ox_x8.data() + i * 8)),
                    ray_rd_x);
            ...
            const __m256 ty1 = _mm256_mul_ps(
                    _mm256_sub_ps(_mm256_broadcast_ss(&bbox.min.y), _mm256_load_ps(rays.oy_x8.data() + i * 8)),
                    ray_rd_y);
            const __m256 ty2 = _mm256_mul_ps(
                    _mm256_sub_ps(_mm256_broadcast_ss(&bbox.max.y), _mm256_load_ps(rays.oy_x8.data() + i * 8)),
                    ray_rd_y);
            ...
            const __m256 tz1 = _mm256_mul_ps(
                    _mm256_sub_ps(_mm256_broadcast_ss(&bbox.min.z), _mm256_load_ps(rays.oz_x8.data() + i * 8)),
                    ray_rd_z);
            const __m256 tz2 = _mm256_mul_ps(
                    _mm256_sub_ps(_mm256_broadcast_ss(&bbox.max.z), _mm256_load_ps(rays.oz_x8.data() + i * 8)),
                    ray_rd_z);
