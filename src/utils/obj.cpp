...
// Visitor class that interprets parsed OBJ statements, to turn them into getTriangle soup
    ...
    public:
        ...
        {
            _triangles.emplace_back(static_cast<int>(_triangles.size()), _vertices[face[0] - 1], _vertices[face[1] - 1],
                                    _vertices[face[2] - 1], _currentMaterial);

            if (face.size() == 4)
            {
                _triangles.emplace_back(static_cast<int>(_triangles.size()), _vertices[face[0] - 1],
                                        _vertices[face[2] - 1], _vertices[face[3] - 1], _currentMaterial);
            }
        }
        ...
        try
        {
            ...
            if (parsed && (it == obj_text.end()))
            {
                ...
                std::for_each(statements.begin(), statements.end(),
                              [&obj_interpreter](const Obj::Statement &s) { std::visit(obj_interpreter, s); });
                ...
                if (normalize)
                {
                    ...
                    std::transform(triangles.begin(), triangles.end(), triangles.begin(),
                                   [center, max_dim](core::Triangle &triangle)
                                   {
